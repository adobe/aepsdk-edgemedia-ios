...
class MediaEventTracker: MediaEventTracking {
    ...
    enum ErrorMessage: String {
        ...
        case ErrInvalidErrorId = "ErrorId passed into 'API:trackError' is invalid. Please pass valid non-empty non-nil string for ErrorId."
    ...
    private let eventProcessor: MediaEventProcessing
    private var xdmEventGenerator: MediaXDMEventGenerator?
    ...
    private static let INVALID_TS: TimeInterval = -1
    private var prerollRefTS: TimeInterval = INVALID_TS
    private var contentStartRefTS: TimeInterval = INVALID_TS
    private var mediaSessionStartTS: TimeInterval = INVALID_TS
    private var mediaIdleStartTS: TimeInterval = INVALID_TS
    ...
    private func setupRules() {
        ...
        error.addPredicate(predicateFn: isInMedia(rule:context:), expectedValue: true, errorMsg: ErrorMessage.ErrNotInMedia.rawValue)
            .addPredicate(predicateFn: isValidErrorInfo(rule:context:), expectedValue: true, errorMsg: ErrorMessage.ErrInvalidErrorId.rawValue)
    ...
    private func isBuffering(rule: MediaRule, context: [String: Any]) -> Bool {
        return mediaContext?.isInMediaPlaybackState(state: .Buffer) ?? false
    }

    private func isSeeking(rule: MediaRule, context: [String: Any]) -> Bool {
        return mediaContext?.isInMediaPlaybackState(state: .Seek) ?? false
    }
    ...
    private func isValidErrorInfo(rule: MediaRule, context: [String: Any]) -> Bool {
        guard let errorInfo = context[Self.KEY_INFO] as? [String: Any] else {
            return false
        }

        guard let errorId = errorInfo[MediaConstants.ErrorInfo.ID] as? String, !errorId.isEmpty else {
            return false
        }
    ...
    private func cmdEnterAction(rule: MediaRule, context: [String: Any]) -> Bool {
        xdmEventGenerator?.setRefTS(getRefTS(context: context))
    ...
    private func cmdExitAction(rule: MediaRule, context: [String: Any]) -> Bool {
        guard let mediaContext = mediaContext else {
            // the mediaContext instance is expected to nil for trackComplete and trackSessionEnd
            // fail for rest of the events
            if rule.name != RuleName.MediaComplete.rawValue && rule.name != RuleName.MediaSkip.rawValue {
                Log.trace(label: Self.LOG_TAG, "cmdExitAction - Cannot process (\(rule.description)) event since media context is nil")
                return false
            }
        ...
        // Force the state to play when adstart is received before any play/pause.
        // Happens usually for preroll ad. Manually switch state to play as the backend
        ...
        // If BufferComplete / SeekComplete is called before first play / pause,
        // manually switch to pause as there is not way to go back to init state.
        ...
        xdmEventGenerator?.processPlayback(doFlush: shouldFlush)
    ...
    private func cmdMediaStart(rule: MediaRule, context: [String: Any]) -> Bool {
        ...
        guard let mediaContext = mediaContext else {
            Log.trace(label: Self.LOG_TAG, "cmdMediaStart - Cannot process SessionStart event since media context is null")
            return false
        }

        xdmEventGenerator = MediaXDMEventGenerator(context: mediaContext, eventProcessor: eventProcessor, config: trackerConfig ?? [:], refEvent: refEvent, refTS: refTS)
        xdmEventGenerator?.processSessionStart()
    ...
    private func cmdAdBreakSkip(rule: MediaRule, context: [String: Any]) -> Bool {
        // This may be called even when not in adbreak
        if mediaContext?.adBreakInfo != nil {
            mediaContext?.clearAdBreakInfo()
    ...
    private func cmdAdSkip(rule: MediaRule, context: [String: Any]) -> Bool {
        // This may be called even when not in ad
        if mediaContext?.adInfo != nil {
            mediaContext?.clearAdInfo()
    ...
    private func cmdChapterStart(rule: MediaRule, context: [String: Any]) -> Bool {
        ...
        mediaContext?.setChapterInfo(chapterInfo, metadata: metadata)
    ...
    private func cmdChapterSkip(rule: MediaRule, context: [String: Any]) -> Bool {
        // This may be called even when not in chapter
        if mediaContext?.chapterInfo != nil {
            mediaContext?.clearChapterInfo()
    ...
    private func cmdError(rule: MediaRule, context: [String: Any]) -> Bool {
        if let errorId = getError(context: context) {
            xdmEventGenerator?.processError(errorId: errorId)
        }
        return true
    }
    ...
    private func cmdStateStart(rule: MediaRule, context: [String: Any]) -> Bool {
        ...
        xdmEventGenerator?.processStateStart(stateInfo: stateInfo)
    ...
    private func cmdStateEnd(rule: MediaRule, context: [String: Any]) -> Bool {
        ...
        xdmEventGenerator?.processStateEnd(stateInfo: stateInfo)
    ...
    private func cmdIdleDetection(rule: MediaRule, context: [String: Any]) -> Bool {
        ...
        if mediaContext.isIdle() {
            ...
            if trackerIdle {
                // Resume tracking if tracking was stopped
                xdmEventGenerator?.processSessionRestart()
                trackerIdle = false
                // Reset content started flag if media is idle
                contentStarted = false
                contentStartRefTS = Self.INVALID_TS
                mediaSessionStartTS = Self.INVALID_TS
            }
    ...
    // Check if there is a need to wait for PrerollWaitTime for preroll ads before executing any track calls
    func prerollDeferRule(rule: RuleName, context: [String: Any]) -> Bool {
        guard  let mediaContext = mediaContext, inPrerollInterval else {
            return false
        }
        let prerollWaitingtimeMS = mediaContext.mediaInfo.prerollWaitingTime
        // Queue the events and stop further downstream processing for preroll_waiting_time ms
