...
class FunctionalTestBase: XCTestCase {
    ...
    func unregisterInstrumentedExtension() {
        let event = Event(name: "Unregister Instrumented Extension",
                          type: FunctionalTestConstant.EventType.INSTRUMENTED_EXTENSION,
                          source: FunctionalTestConstant.EventSource.UNREGISTER_EXTENSION,
                          data: nil)
    ...
    func assertExpectedEvents(ignoreUnexpectedEvents: Bool = false, file: StaticString = #file, line: UInt = #line) {
        ...
        for expectedEvent in currentExpectedEvents {
            let waitResult = expectedEvent.value.await(timeout: FunctionalTestConstant.Defaults.WAIT_EVENT_TIMEOUT)
    ...
    func assertUnexpectedEvents(file: StaticString = #file, line: UInt = #line) {
        ...
        for receivedEvent in currentReceivedEvents {
            ...
            if let expectedEvent = InstrumentedExtension.expectedEvents[EventSpec(type: receivedEvent.key.type, source: receivedEvent.key.source)] {
                _ = expectedEvent.await(timeout: FunctionalTestConstant.Defaults.WAIT_EVENT_TIMEOUT)
    ...
    func getDispatchedEventsWith(type: String, source: String, timeout: TimeInterval = FunctionalTestConstant.Defaults.WAIT_EVENT_TIMEOUT, file: StaticString = #file, line: UInt = #line) -> [Event] {
        if InstrumentedExtension.expectedEvents[EventSpec(type: type, source: source)] != nil {
            ...
            wait(FunctionalTestConstant.Defaults.WAIT_TIMEOUT)
        }
    ...
    func getSharedStateFor(_ ownerExtension: String, timeout: TimeInterval = FunctionalTestConstant.Defaults.WAIT_SHARED_STATE_TIMEOUT) -> [AnyHashable: Any]? {
        log("GetSharedState for \(ownerExtension)")
        let event = Event(name: "Get Shared State",
                          type: FunctionalTestConstant.EventType.INSTRUMENTED_EXTENSION,
                          source: FunctionalTestConstant.EventSource.SHARED_STATE_REQUEST,
    ...
    /// - See also:
    ///     - setExpectationNetworkRequest(url:httpMethod:)
    func getNetworkRequestsWith(url: String, httpMethod: HttpMethod, timeout: TimeInterval = FunctionalTestConstant.Defaults.WAIT_NETWORK_REQUEST_TIMEOUT, file: StaticString = #file, line: UInt = #line) -> [NetworkRequest] {
        guard let requestUrl = URL(string: url) else {
            assertionFailure("Unable to convert the provided string \(url) to URL")
            return []
        }
    ...
    /// Returns all the `NetworkRequest`(s) sent through the Core NetworkService, or empty if none was found.
    /// - Returns: list of all the dispatched network requests or empty if none was dispatched
    func getAllNetworkRequests() -> [NetworkRequest] {
        // Wait for network requests to be dispatched
        wait(FunctionalTestConstant.Defaults.WAIT_TIMEOUT)

        return FunctionalTestBase.networkService.getAllReceivedNetworkRequests()
    }
    ...
    func getNetworkRequestBodyAsDictionary(_ networkRequest: NetworkRequest) -> [String: Any] {
        if !networkRequest.connectPayload.isEmpty {
            if let payloadAsDictionary = try? JSONSerialization.jsonObject(with: networkRequest.connectPayload, options: []) as? [String: Any] {
                return payloadAsDictionary
            } else {
                XCTFail("Failed to parse networkRequest.connectionPayload to JSON")
            }
        }
