...
class FunctionalTestNetworkService: NetworkService {
    private var allReceivedNetworkRequests: [NetworkRequest] = []
    ...
    override func connectAsync(networkRequest: NetworkRequest, completionHandler: ((HttpConnection) -> Void)? = nil) {
        ...
        // Add to the single list of all network requests
        allReceivedNetworkRequests.append(networkRequest)
    ...
    func reset() {
        allReceivedNetworkRequests.removeAll()
    ...
    func getAllReceivedNetworkRequests() -> [NetworkRequest] {
        return allReceivedNetworkRequests
    }
