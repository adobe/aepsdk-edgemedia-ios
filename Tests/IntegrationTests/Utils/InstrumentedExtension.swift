...
class InstrumentedExtension: NSObject, Extension {
    ...
    func wildcardListenerProcessor(_ event: Event) {
        if event.type.lowercased() == FunctionalTestConstant.EventType.INSTRUMENTED_EXTENSION.lowercased() {
            // process the shared state request event
            if event.source.lowercased() == FunctionalTestConstant.EventSource.SHARED_STATE_REQUEST.lowercased() {
                processSharedStateRequest(event)
            }
            // process the unregister extension event
            else if event.source.lowercased() == FunctionalTestConstant.EventSource.UNREGISTER_EXTENSION.lowercased() {
                unregisterExtension()
            }
    ...
    func processSharedStateRequest(_ event: Event) {
        ...
        guard let owner = eventData[FunctionalTestConstant.EventDataKey.STATE_OWNER] as? String else { return }

        var responseData: [String: Any?] = [FunctionalTestConstant.EventDataKey.STATE_OWNER: owner, FunctionalTestConstant.EventDataKey.STATE: nil]
        if let state = runtime.getSharedState(extensionName: owner, event: event, barrier: false) {
            responseData[FunctionalTestConstant.EventDataKey.STATE] = state
        }
        ...
        let responseEvent = event.createResponseEvent(name: "Get Shared State Response",
                                                      ...
                                                      source: FunctionalTestConstant.EventSource.SHARED_STATE_RESPONSE,
