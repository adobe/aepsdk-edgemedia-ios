...
class MediaEventTrackerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        mediaTracker = MediaEventTracker(eventProcessor: MediaEventProcessor(dispatcher: nil), config: Self.config)
    }
    ...
    func testTrackError_WithValidString_passes() {
        eventGenerator.trackSessionStart(info: Self.media!.toMap(), metadata: Self.metadata)
        XCTAssertTrue(handleTrackAPI())
    ...
    func testTrackError_WithEmtpyString_fails() {
        eventGenerator.trackSessionStart(info: Self.media!.toMap(), metadata: Self.metadata)
        XCTAssertTrue(handleTrackAPI())

        eventGenerator.trackError(errorId: "")
        XCTAssertFalse(handleTrackAPI())
    }
